// $Id: ASBeautifier.cpp,v 1.28 2005/06/15 16:42:25 devsolar Exp $
// --------------------------------------------------------------------------
//
...
namespace astyle
  ...
  static const string * headers_[] =
    {
      &AS_IF, &AS_ELSE, &AS_FOR, &AS_WHILE,
  ...
  static const string * nonParenHeaders_[] =
    {
      &AS_ELSE, &AS_DO, &AS_TRY, &AS_FINALLY,
  ...
  static const string * preBlockStatements_[] =
    {
      &AS_CLASS, &AS_STRUCT, &AS_UNION,
  ...
  static const string * assignmentOperators_[] =
    {
      &AS_ASSIGN, &AS_PLUS_ASSIGN,
  ...
  static const string * nonAssignmentOperators_[] =
    {
      &AS_EQUAL, &AS_PLUS_PLUS,
    ...
    if ( (sourceStyle != STYLE_JAVA) && !isInComment && (line[0] == '#' || backslashEndsPrevLine))
      {
              ...
              if (BEGINS_WITH(preproc, "if", 2))
                ...
                if (BEGINS_WITH(preproc, "else", 4))
                  ...
                  if (BEGINS_WITH(preproc, "elif", 4))
                    ...
                    if (BEGINS_WITH(preproc, "endif", 5))
                      {
        ...
        if (line.size() > 0)
          backslashEndsPrevLine = (line[line.size() - 1] == '\\');
        else
          backslashEndsPrevLine = false;
    ...
    for (i = 0; i < headerStackSize; i++)
      {
        ...
        if ( ( sourceStyle != STYLE_JAVA ) && !namespaceIndent && i >= 1
                && (*headerStack)[i - 1] == &AS_NAMESPACE
                && (*headerStack)[i] == &AS_OPEN_BRACKET)
          --tabCount;

        if (( sourceStyle != STYLE_JAVA ) && i >= 1
    ...
    for (i = 0; i < line.size(); i++)
      {
        ...
        if ( !(isInComment || isInLineComment) && CONTAINS_AT(line, AS_OPEN_LINE_COMMENT, 2, i) )
          ...
          if ( !(isInComment || isInLineComment) && CONTAINS_AT(line, AS_OPEN_COMMENT, 2, i) )
            {
              isInComment = true;
              outBuffer.append(1, '*');
              i++;
            ...
            if ( (isInComment || isInLineComment) && CONTAINS_AT(line, AS_CLOSE_COMMENT, 2, i) )
              {
                isInComment = false;
                outBuffer.append(1, '/');
                i++;
                      ...
                      if (newHeader == &AS_CATCH || newHeader == &AS_FINALLY)
                        ...
                        if (newHeader == &AS_CASE)
                          ...
                          if (newHeader == &AS_DEFAULT)
                            ...
                            if (newHeader == &AS_PUBLIC || newHeader == &AS_PROTECTED || newHeader == &AS_PRIVATE)
                              {
        ...
        if (( sourceStyle != STYLE_JAVA ) && !isalpha(prevCh)
                && CONTAINS_AT(line, AS_OPERATOR, 8, i) && !isalnum(line[i + 8]))
          {
            isInOperator = true;
        ...
        if (ch == ':')
          {
            if (line.size() > i + 1 && line[i + 1] == ':') // look for ::
              ...
              if (( sourceStyle != STYLE_JAVA ) && isInClass && prevNonSpaceCh != ')')
                {
                  --tabCount;
                ...
                if (( sourceStyle != STYLE_JAVA ) && isInClassHeader)
                  ...
                  if (isInQuestion)
                    ...
                    if (( sourceStyle != STYLE_JAVA ) && prevNonSpaceCh == ')')
                      {
            ...
            if (ch == '/')
              {
                if (CONTAINS_AT(line, AS_OPEN_LINE_COMMENT, 2, i + charDistance))
                  ...
                  if (CONTAINS_AT(line, AS_OPEN_COMMENT, 2, i + charDistance))
                    {
                      charDistance++;
                      inComment = true;
                    }
