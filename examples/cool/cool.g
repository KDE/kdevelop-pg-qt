...
-> feature ;;
 ...
 | ?[: LA(2).kind == Token_LPAREN :]     name=ID LPAREN #argument=expression @ COMMA RPAREN       -- dispatch
 | variable=ID
...
-> postfix_expression ;;
   ...
   IF condition=expression THEN true_expression=expression ELSE false_expression=expression
-> if_expression ;;
...
-> let_expression ;;
   ...
   CASE expression=expression OF !(#condition=case_condition SEMICOLON) IN body_expression=expression
-> case_expression ;;
