  ...
  do
    {
      ...
      java::token_type &t = this->token_stream->next();
      t.kind = kind;
...
void java::report_problem( java::problem_type type, std::string message )
{
  report_problem( type, message.c_str() );
}

void java::report_problem( java::problem_type type, const char* message )
{
  if (type == error)
