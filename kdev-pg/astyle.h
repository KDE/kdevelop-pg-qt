// $Id: astyle.h,v 1.29 2005/06/15 16:42:26 devsolar Exp $
// --------------------------------------------------------------------------
//
...
#if defined( NDEBUG )
#define TRACE( type, x ) ( (void) 0 )
#define TRACE_LIFE ( (void) 0 )
#else
#define TRACE( type, x ) { std::ostringstream msg; msg << x; Tracer::out( msg.str(), ERRLOC, type ); }
#define TRACE_LIFE Tracer __astyle_tracer( ERRLOC )
#endif
...
namespace astyle
  ...
  class Tracer
    {
    public:
      Tracer( string const & file, string const & func, int const line )
