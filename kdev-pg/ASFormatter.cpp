// $Id: ASFormatter.cpp,v 1.29 2005/06/15 16:42:26 devsolar Exp $
// --------------------------------------------------------------------------
//
...
namespace astyle
  ...
  static const string * headers_[] =
    {
      &AS_IF, &AS_ELSE, &AS_DO, &AS_WHILE, &AS_FOR,
  ...
  static const string * preDefinitionHeaders_[] =
    {
      &AS_CLASS, &AS_INTERFACE,
  ...
  static const string * preCommandHeaders_[] =
    {
      &AS_EXTERN, &AS_THROWS, &AS_CONST
    };
  ...
  static const string * preprocessorHeaders_[] =
    {
      &AS_BAR_DEFINE
  ...
  static const string * operators_[] =
    {
      &AS_PLUS_ASSIGN, &AS_MINUS_ASSIGN,
    ...
    while (!isLineReady)
      {
        ...
        if (isSequenceReached(AS_OPEN_LINE_COMMENT))
          ...
          if (isSequenceReached(AS_OPEN_COMMENT))
            {
              isInComment = true;
              if (padOperators)
                appendSpacePad();
                    ...
                    i += (oper ? oper->length() : 1) )
              {
                ...
                if (oper == &AS_LS)
                  ...
                  if (oper == &AS_GR)
                    ...
                    if (oper == &AS_COMMA               // comma,     e.g. A<int, char>
                      ...
                      if (!isLegalNameChar(currentLine[i]) && !isWhiteSpace(currentLine[i]))
                        {
        ...
        if (!isInTemplate)
          {
            if ( (newHeader = findHeader(headers)) != NULL)
              ...
              if ( (newHeader = findHeader(preDefinitionHeaders)) != NULL)
                {
                ...
                if ( (newHeader = findHeader(preCommandHeaders)) != NULL)
                  {
